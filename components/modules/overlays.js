/**
 * OVERLAYS MODULE
 * Modals, Popovers, Tooltips, Dropdowns, Dialogs
 */
...
export const Modal = {
  ...
  template: (props) => {
    ...
    footer.innerHTML = props.actions?.map(action => 
...
export const Dropdown = {
  ...
  template: (props) => {
    ...
    menu.innerHTML = props.items?.map((item, i) => 
      `<li><button class="dropdown-item" data-index="${i}">${item.label}</button></li>`
    ).join('') || '';
