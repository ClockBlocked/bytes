const CodeBlock = {
  ...
  template: (props) => {
    const config = { ...CodeBlock.defaults, ... props };
  ...
  renderViewerContent:  (config) => {
    ...
    const highlightedCode = CodeBlock.highlightSyntax(sourceCode, config. language);
  ...
  highlightSyntax: (code, language) => {
    const escaped = CodeBlock.escapeHTML(code || '');
  ...
  lifecycle: {
    onMount: (element, props) => {
      ...
      if (config.mode === 'viewer') {
        CodeBlock.initViewerMode(element, config);
      } else {
        CodeBlock. initEditorMode(element, config);
      }

      // Common initializations
      CodeBlock.initThemeToggle(element);
      CodeBlock. initModeToggle(element, props);
      CodeBlock. restoreTheme(element);
    }
  },
    ...
    codeElement?.addEventListener('click', () => {
      ...
      if (selection && selection.rangeCount > 0) {
        ...
        if (pre) {
          ...
          CodeBlock.updateCursorPosition(element, offset);
        }
      }
    });
  },
  ...
  initModeToggle: (element, props) => {
    ...
    modeBtn?. addEventListener('click', () => {
      ...
      CodeBlock.switchMode(element, targetMode, props);
    });
  },
  ...
  switchMode: (element, targetMode, props) => {
    ...
    setTimeout(() => {
      ...
      if (header) {
        header.innerHTML = CodeBlock.renderHeader(state);
        CodeBlock.initModeToggle(element, props);
      }
      ...
      if (toolbar) {
        toolbar.innerHTML = CodeBlock.renderToolbar(state);
        CodeBlock.initThemeToggle(element);
      }
      ...
      if (content) {
        content.innerHTML = targetMode === 'viewer'
          ? CodeBlock.renderViewerContent(state)
          : CodeBlock.renderEditorContent(state);
      }

      // Update status bar
      const statusBar = element. querySelector('.code-block-status');
      if (statusBar) {
        statusBar.innerHTML = CodeBlock.renderStatusBar(state);
      }

      // Reinitialize mode-specific functionality
      if (targetMode === 'viewer') {
        CodeBlock.initViewerMode(element, state);
      } else {
        CodeBlock.initEditorMode(element, { ...props, ...state });
      }
  ...
  // Factory method to create and mount
  create: (container, props) => {
    const element = CodeBlock.template(props);
    if (typeof container === 'string') {
      container = document.querySelector(container);
    }
    if (container) {
      container.appendChild(element);
      CodeBlock.lifecycle.onMount(element, props);
    }
    return element;
  }
};
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CodeBlock, CodeBlockStyles };
}
/**
 * 
 *  C R E A T E D  B Y
